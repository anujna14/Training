...
var bigOak = require("../javascript/crowTech").bigOak;

var defineRequestType = require("../javascript/crowTech").defineRequestType;
...
function locateScalpel2(nest: any) {
  function getNextNest(currentNest: string) {
    return anyStorage(nest, currentNest, "scalpel").then((response: any) => {
      if (response === currentNest) {
        return currentNest;
      } else {
        return getNextNest(response);
      }
    });
  }
  return getNextNest(nest.name);
}
